...
const DEFAULT_PROTO: &str = "nats";
const DEFAULT_HOST: &str = "localhost";
const DEFAULT_PORT: u16 = 4222;
const DEFAULT_URL: &str = DEFAULT_PROTO + "://" + DEFAULT_HOST + ":" + DEFAULT_PORT;
const DEFAULT_MAX_RECONNECT: u32 = 60;
const DEFAULT_RECONNECT_WAIT: Duration = 2 * time::SECOND;
const DEFAULT_TIMEOUT: Duration = 2 * time::SECOND;
const DEFAULT_PING_INTERVAL: Duration = 120 * time::SECOND;
const DEFAULT_MAX_PING_OUT: u32 = 2;
const DEFAULT_MAX_CHAN_LEN: usize = 8192;
const DEFAULT_RECONNECT_BUF_SIZE: usize = 8 * 1024 * 1024;
const DEFAULT_REQUEST_CHAN_LEN: usize = 8;
const DEFAULT_DRAIN_TIMEOUT: Duration = 30 * time::SECOND;


const STALE_CONNECTION: &str = "stale connection";
const PERMISSIONS_ERR: &str = "permissions violation";
const AUTHORIZATION_ERR: &str = "authorization violation";
...
type ConnHandler = Fn(&Conn);
type ErrHandler = Fn(&Conn, &Subscription, Error);
type UserJwtHandler = Fn() -> Result<String>;
type SignatureHandler = Fn(&[u8]) -> Result<&[u8]>;
type AuthTokenHandler = Fn() -> String;

type Option = Fn(&Options) -> Error;

trait Dialer {
    fn dial(network, address: std::net::ToSocketAddrs) -> Result<std::net::TcpStream>;
}
...
struct Options {
    ...
    max_ping_out: u32,
    on_closed: ConnHandler,
    on_disconnected: ConnHandler,
    on_reconnected: ConnHandler,
    on_discovered_servers: ConnHandler,
    on_async_error: ErrHandler,
    reconnect_buf_size: usize,
    sub_chan_len: usize,
    on_user_jwt: UserJwtHandler,
    nkey: String,
    on_signature: SignatureHandler,
    user: String,
    password: String,
    token: String,
    on_auth_token: AuthTokenHandler,
    dialer: &Dialer,

}

static SCRATCH_SIZE: usize = 512;
static DEFAULT_BUF_SIZE: usize = 32768;
static FLUSH_CHAN_SIZE: usize = 1024;
static SRV_POOL_SIZE: u32 = 4;
static NUID_SIZE: usize = 22;
static DEFAULT_PORT_STRING: &str = format!("{}", DEFAULT_PORT);
